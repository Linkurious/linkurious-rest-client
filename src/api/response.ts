...
export class LkResponse<B = unknown> {
  ...
  public static isError<R extends LkResponse, E extends LkErrorKey>(
    response: R,
    key: E
  ): response is Extract<R, LkResponse<ILkError<E>>> {
    return (response.body as ILkError).key === key;
  }
}
...
export interface CannotDeleteNonEmptyFolder
  extends ILkError<LkErrorKey.CANNOT_DELETE_NON_EMPTY_FOLDER> {}
